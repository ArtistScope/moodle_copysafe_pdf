...
//$PAGE->set_url('/mod/copysafepdf/function.php', array('id' => $cm->id));
...
function wpcsw_setting_del($nname){
    global $DB,$CFG;
 $name = trim($nname) ;
 $result=$DB->delete_records('copysafepdf_settings',array('name'=>$name)); 
 if($result)
 unlink($CFG->dirroot.'/mod/copysafepdf/copysafepdfimage/'.$name);
 return $result ;
